...
from pyregions.github import tabletools
import re

from pyregions.widgets.table_api import TableApi
...
def getReport(report_key: str, filename: str) -> ConfigurationTypes:
	...
	elif report_key in {'WDI', 'World Development Indicators'}:
		report_configuration = getWorldDevelopmentIndicatorsConfiguration(filename)
	else:
		raise NotImplementedError

	parsed_table = TableApi(filename, **report_configuration)
...
def getWorldEconomicOutlookConfiguration() -> ConfigurationTypes:
	...
	if configuration is None:
		...
		configuration = {
			...
			# 'saveTo': r"C:\Users\Progi\Documents\GitHub\Regions\pyregions\test_api.json"
		}
...
def getWorldDevelopmentIndicatorsConfiguration(filename: Optional[str]):
	configuration_filename: str = getFilename('wdi_configuration')
	configuration: Dict = loadConfiguration(configuration_filename)
	if filename is None:
	...
	if configuration is None:
		...
		series_description_map: Dict[str, str] = dict()
		series_tag_map: Dict[str, str] = dict()

		def _getWDIUnits(unitstring: str):

			# string = [i.strip() for i in string.split('\n')]
			regex: Pattern = re.compile("\(([^)]+)\)$")
			match: Match = regex.search(unitstring)
			if match:
		...
		for index, row in enumerate(series_sheet):
			...
			_tags:List[str] = ['|'.join(i) for i in _tags]
			series_tag_map[subject_code] = _tags
			series_unit_map[subject_code] = _unit

		series_notes_map: Dict[str, str] = dict()
...
def getAmericanStateConfiguration(filename: str)->ConfigurationTypes:
	configuration_filename = getFilename('american_population_configuration')
	configuration = loadConfiguration(configuration_filename)
	...
	raise NotImplementedError
def getAmericanCensusTractResponse(filename:str):
	#countryCode	countryName	landArea	regionCode	regionDensity	regionName	source	stateCode	stateFips	totalPopulation	url	waterArea
	...
	for index, row in table.iterrows():
		...
		common_notes = 'Values were extracted from shapefiles supplied by the US census bureau.'
		pop_series = {
			'seriesName': 'Population',
			'seriesCode': 'POP',
			'seriesDescription': "Total population counted during the 2010 census.",
			'seriesUnits': 'Persons',
			'seriesScale': 'unit',
			'seriesNotes': common_notes,
			'seriesValues': [('2010-07-01', row['totalPopulation'])]
		}
		land_series = {
			'seriesName': 'Land Area',
			'seriesCode': 'LAREA',
			'seriesDescription': "Total land area.",
			'seriesUnits': '',
			'seriesScale': '',
			'seriesNotes': common_notes,
			'seriesValues': [('2010-07-01', row['landArea'])]
		}
		water_series = {
			'seriesName': 'Water Area',
			'seriesCode': 'WAREA',
			'seriesDescription': '',
			'seriesUnits': '',
			'seriesScale': '',
			'seriesNotes': common_notes,
			'seriesValues': [('2010-07-01', row['waterArea'])]
		}
...
if __name__ == "__main__":
	fname = ""
	getMaddisonPopulationConfiguration()